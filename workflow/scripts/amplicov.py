...
def covert_bedpe_to_amplicon_dict(input,unique=False):
    ...
    for line in proc.stdout:
         ...
         if (previous_id and unique):
            ...
            #except:
                #id = previous_id
                #print("possible duplicate amplicon")
            #print(set_new)
            #print(set_previous)
            #print(set_new_update)
            #print(set_previous_update)
            #print(id + " " + start + " " + end)
         previous_id = id    
...
def write_dict_to_file(mean_d,uniq_mean_d,outdir,prefix):
    ...
    with open(output_amplicon_cov_txt,"w") as f:
        ...
        for key in mean_d:
            f.write("%s\t%.2f\t%.2f\n" % (key,mean_d.get(key, 0.00),uniq_mean_d.get(key, 0.00)))
...
def barplot(mean_dict,uniq_mean_d,input_bed,overall_mean,outdir,prefix):
    ...
    fig = go.Figure(data=[go.Bar(x=uniq_x, y=uniq_y)])
    fig.add_trace(go.Bar(x=x,y=y,marker_color='lightsalmon',visible=False))
    ...
    depth200x = [dict(type='line',
                    xref='paper',x0=0,x1=1,
                    yref='y',y0=200,y1=200,
                    line=dict(
                        color="black",
                        width=1,
                        dash='dot',
    ...
    updatemenus = list([
        ...
        dict(
            buttons=list([
                ...
                dict(label="50X",
                 method="relayout",
                 args=["shapes", depth50x]),
                dict(label="100X",
                 method="relayout",
                 args=["shapes", depth100x]),
                dict(label="200X",
                 method="relayout",
                 args=["shapes", depth200x]),
                dict(label="All",
                 method="relayout",
                 args=["shapes", depthMean + depth50x + depth100x + depth200x])
                ]),
             direction="down",
             x=0.30,
             xanchor="left",
...
def run(argvs):
    ...
    if (argvs.bedpe):
        ...
        uniq_amplicon_dict = covert_bedpe_to_amplicon_dict(argvs.bedpe,True)
        bedfile = argvs.bedpe
    if (argvs.cov):
